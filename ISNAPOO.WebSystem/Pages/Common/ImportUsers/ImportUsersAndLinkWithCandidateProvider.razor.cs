...
{
    public partial class ImportUsersAndLinkWithCandidateProvider : BlazorBaseComponent
    {
        ...
        private KeyValueVM CPO;
        private KeyValueVM CIPO;
        private KeyValueVM ExternalExpert;
        private KeyValueVM NAPOOExpert;
        private bool Enable;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (await this.IsInRole("ADMIN"))
            {
                ...
                this.CPO = importUsersTypeSource.Where(x => x.KeyValueIntCode.Equals("CPO")).First();
                this.CIPO = importUsersTypeSource.Where(x => x.KeyValueIntCode.Equals("CIPO")).First();
                this.ExternalExpert = importUsersTypeSource.Where(x => x.KeyValueIntCode.Equals("ExternalExperts")).First();
                this.NAPOOExpert = importUsersTypeSource.Where(x => x.KeyValueIntCode.Equals("NAPOOExperts")).First();
        ...
        private async Task ImportBtn()
        {
            ...
            if (!isFileUploaded && (this.model.IdImportType == this.CPO.IdKeyValue || this.model.IdImportType == this.CIPO.IdKeyValue))
            {
            ...
            try
            {
                ...
                {
                        var resultContext = await this.ImportUserService.ImportUsersAndLinkToCandidateProviderAsync(this.model.UploadedFileStream, this.model.UploadedFileName, this.model.IdImportType!.Value);
                        ...
                        {
                            var result = await this.ImportUserService.CreateUsersForCandidateProvidersAsync(resultContext.ResultContextObject, this.model);
                            if (result.IsSuccessfull)
                            {
                                await this.ShowSuccessAsync("Импортът приключи успешно!");
                                await this.JsRuntime.SaveAs($"Users_import_{DateTime.Now.ToString(GlobalConstants.DATE_FORMAT_DELETED_FILE)}.xlsx", result.MS.ToArray());
        ...
        private void OnChangeValue(ChangeEventArgs<int?, KeyValueVM> args)
        {
            if (args.Value == CPO.IdKeyValue || args.Value == CIPO.IdKeyValue)
                Enable = true;
            else
                Enable = false;
        }
