...
{
    public partial class DOCERUModal : BlazorBaseComponent
    {
        ...
        private List<TrainingCurriculumVM> trainingCurriculums = new List<TrainingCurriculumVM>();
        ...
        public async Task OpenModal(SpecialityVM speciality, List<CandidateCurriculumVM> selectedCurriculums = null, List<TrainingCurriculumVM> trainingCurriculums = null)
        {
            ...
            {
                this.trainingCurriculums = trainingCurriculums;
            }
        ...
        private async Task SetERUsData(SpecialityVM speciality, DocVM doc)
        {
            ...
            foreach (var eru in this.eruDataSource)
            {
                if (eru.IdNKRLevel != 0)
                {
                    var nkrLevelValue = this.kvNKRLeveSource.FirstOrDefault(x => x.IdKeyValue == eru.IdNKRLevel);
                    if (nkrLevelValue is not null)
                    {
                        eru.NKRLevelName = nkrLevelValue.Name;
                    }
                }

                if (eru.IdEKRLevel != 0)
                {
                    var ekrLevelValue = this.kvEKRLeveSource.FirstOrDefault(x => x.IdKeyValue == eru.IdEKRLevel);
                    if (ekrLevelValue is not null)
                    {
                        eru.EKRLevelName = ekrLevelValue.Name;
                    }
                }
            }
        }
        ...
        private async Task AddSelectedERUsHandler()
        {
            ...
            else
            {
                ...
                {
                    result = await this.TrainingService.AddERUsToCurriculumListAsync(this.selectedERUs, this.trainingCurriculums);
                }
