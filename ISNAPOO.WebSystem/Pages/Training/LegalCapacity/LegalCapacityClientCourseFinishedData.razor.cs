...
{
    public partial class LegalCapacityClientCourseFinishedData : BlazorBaseComponent
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            this.kvSPKId = (await this.DataSourceService.GetKeyValueByIntCodeAsync("TypeFrameworkProgram", "ProfessionalQualification")).IdKeyValue;
        ...
        public override async void SubmitHandler()
        {
            ...
            this.model.IdDocumentType = this.kvSPKId;
                ...
                if (this.CourseVM is not null)
                {
                    if (this.CourseVM.StartDate.HasValue)
                    {
                        if (this.model.DocumentDate < this.CourseVM.StartDate)
                        {
                            ...
                            this.messageStore?.Add(fi, $"Полето 'Дата на издаване (документ с фабричен номер)' не може да бъде преди {this.CourseVM.StartDate.Value.ToString("dd.MM.yyyy")}г.!");
                        }
                    }

                    if (this.model.LegalCapacityDocumentDate < this.CourseVM.StartDate)
                    {
                        FieldIdentifier fi = new FieldIdentifier(this.model, "FinishedDate");
                        this.messageStore?.Add(fi, $"Полето 'Дата на издаване (документ за правоспособност)' не може да бъде преди {this.CourseVM.StartDate.Value.ToString("dd.MM.yyyy")}г.!");
                    }
                }
            }
        }
        ...
        private void ValidateRequiredFields(object? sender, ValidationRequestedEventArgs args)
        {
            if (this.model.IdFinishedType.HasValue && this.model.IdFinishedType.Value == this.kvFinishedWithDoc.IdKeyValue)
            {
                ...
                if (this.model.DocumentDate is null)
                {
                    ...
                    this.messageStore?.Add(fi, "Полето 'Дата на издаване (документ с фабричен номер)' е задължително!");
                }
