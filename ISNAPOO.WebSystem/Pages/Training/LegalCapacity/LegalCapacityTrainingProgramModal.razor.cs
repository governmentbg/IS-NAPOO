...
{
    public partial class LegalCapacityTrainingProgramModal : BlazorBaseComponent
    {
        ...
        private bool showLegalCapacityOrdinanceType = true;
        ...
        public async Task OpenModal(ProgramVM program, int idCourseType, ConcurrencyInfo concurrencyInfo = null)
        {
            this.SpinnerShow();
        ...
        private async Task SubmitBtn()
        {
            ...
            try
            {
                ...
                if (this.programVM.IdSpeciality != 0 && this.programVM.IdFrameworkProgram != 0 && !string.IsNullOrEmpty(this.programVM.ProgramNumber))
                {
                ...
                if (!this.validationMessages.Any())
                {
                    ...
                    if (this.programVM.IdProgram == 0)
                    {
                        result = await this.TrainingService.CreateTrainingProgramAsync(inputContext, this.idCourseType);
        ...
        private async Task CreateUpcomingCourseBtn()
        {
            ...
            {
                await this.ShowErrorAsync("Не можете да създадете предстоящ курс за правоспособност за специалност, която е отпаднала от СППОО!");
                return;
            }
            ...
            try
            {
                ...
                if (this.programVM.IdFrameworkProgram.HasValue && this.programVM.FrameworkProgram is not null && this.programVM.FrameworkProgram.IdStatus.HasValue && this.programVM.FrameworkProgram.IdStatus.Value != this.kvActiveStatus.IdKeyValue)
                {
                    await this.ShowErrorAsync("Не можете да създадете курс за правоспособност за стара рамкова програма!");
                }
                else
                {
                    var formEducationSource = await this.DataSourceService.GetKeyValuesByKeyTypeIntCodeAsync("FormEducation");
                    await this.upcomingTrainingCourseModal.OpenModal(new CourseVM() { IdCandidateProvider = this.UserProps.IdCandidateProvider, CourseName = this.programVM.ProgramName }, formEducationSource, this.idCourseType, isTrainingProgramAlreadySelected: true);
