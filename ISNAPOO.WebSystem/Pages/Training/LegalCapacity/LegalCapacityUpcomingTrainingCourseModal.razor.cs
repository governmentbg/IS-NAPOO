...
{
    public partial class LegalCapacityUpcomingTrainingCourseModal : BlazorBaseComponent
    {
        ...
        public async Task OpenModal(CourseVM course, IEnumerable<KeyValueVM> formEducationSource, int idCourseType, ConcurrencyInfo concurrencyInfo = null, bool isTrainingProgramAlreadySelected = false)
        {
            await this.LoadKVDataAsync();
            if (isTrainingProgramAlreadySelected)
            {
                this.courseVM.IdCandidateProvider = course.IdCandidateProvider;
            }
            else
            {
        ...
        private async Task SubmitBtn(bool showToast)
        {
            ...
            try
            {
                ...
                if (this.editContext.Validate())
                {
                    ...
                    {
                        await this.ShowErrorAsync("Не можете да създадете предстоящ курс за правоспособност, тъй като в момента сте с временно отнета лицензия!");
                        this.SpinnerHide();
                        return;
                    }
                    ...
                    {
                        await this.ShowErrorAsync($"Не можете да запишете предстоящ курс за правоспособност с 'Дата на завършване на курса' {this.courseVM.EndDate!.Value.Year} г., защото има данни за подаден годишен отчет за {this.courseVM.EndDate!.Value.Year} г.!");
                        this.SpinnerHide();
                        return;
                    }
                    ...
                    {
                        await this.ShowErrorAsync($"Не можете да създадете предстоящ курс за правоспособност за специалност, която е отпаднала от Списъка на професиите за професионално образование и обучение!");
                        this.SpinnerHide();
                        return;
                    }
        ...
        private async Task OpenSelectProgramModal()
        {
            ...
            try
            {
                ...
                await this.selectTrainingProgramModal.OpenModal(this.courseVM, this.professionalTrainingSource, this.typeFrameworkProgramSource, this.vqsSource, this.idCourseType, true);
            }
            finally
            {
                this.loading = false;
        ...
        {
            string msg = "Сигурни ли сте, че искате да стартирате предстоящия курс?";
            ...
            if (isConfirmed)
            {
                ...
                try
                {
                    ...
                    if (!this.editContext.GetValidationMessages().Any())
                    {
                        if (await this.IsSpecialityFromSPPOORemovedWithOrderBeforeCourseStartDateAsync())
                        {
                            await this.ShowErrorAsync($"Не можете да стартирате курс за правоспособност за специалност, която е отпаднала от Списъка на професиите за професионално образование и обучение!");
                            this.SpinnerHide();
                            return;
                        }
