...
{
    public partial class CurrentTrainingCourseModal : BlazorBaseComponent, IConcurrencyCheck<CourseVM>
    {
        ...
        private CurrentCourseClients currentCourseClients = new CurrentCourseClients();
        ...
        private CourseCurriculumSchedule courseCurriculumSchedule = new CourseCurriculumSchedule();
        private VerificationSubmission verificationSubmission = new VerificationSubmission();
        ...
        private List<int> clientCourseIds = null;
        ...
        public override bool IsContextModified => this.isEditable ? this.currentCourseInformation.IsEditContextModified() || this.currentCourseExam.IsEditContextModified() : false;
        ...
        public async Task OpenModal(CourseVM course, ConcurrencyInfo concurrencyInfo = null, bool isEditable = true, List<int> clientCourseIds = null)
        {
            this.selectedTab = 0;
            ...
            var type = this.courseVM.IdTrainingCourseType == this.kvCourseTypeSPK.IdKeyValue
                ? "SPK"
                : "PP";
            ...
            else if (type.Equals("All") && isEditable == false)
            {
                this.hideBtnsWhenSPK = true;
            }

            this.clientCourseIds = clientCourseIds;
            ...
            if (courseVM.IsArchived)
            {
                modalType = "архивиран";
            }
            else if (courseVM.IdStatus == this.kvCompletedCourse.IdKeyValue)
            {
                modalType = "приключил";
            }
            else if (courseVM.IdStatus == this.kvUpcomingCourse.IdKeyValue)
            {
                modalType = "предстоящ";
            }
            else if (courseVM.IdStatus == this.kvCurrentCourse.IdKeyValue)
            {
                modalType = "текущ";
            }
        ...
        private async Task ExportPdf()
        {
            ...
            dataTable.Rows.Add(new object[] { "Наименование на програмата:", this.courseVM.Program == null ? "" : this.courseVM.Program.ProgramName }); ;
            dataTable.Rows.Add(new object[] { "Вид на курса:", this.courseVM.TrainingCourseTypeName == null ? "" : this.courseVM.TrainingCourseTypeName });
            dataTable.Rows.Add(new object[] { "Допълнителни бележки:", this.courseVM.Program == null ? "" : this.courseVM.Program.ProgramNote });
            ...
            dataTable.Rows.Add(new object[] { "Вид:", this.courseVM.MeasureType == null ? "" : this.courseVM.MeasureType.Name });
            dataTable.Rows.Add(new object[] { "Статус на курса:", this.courseVM.Status == null ? "" : this.courseVM.Status.Name });
            dataTable.Rows.Add(new object[] { "Основен източник на финансиране:", this.courseVM.AssignType == null ? "" : this.courseVM.AssignType.Name });
            dataTable.Rows.Add(new object[] { "Форма на обучение:", this.courseVM.FormEducation == null ? "" : this.courseVM.FormEducation.Name });
            dataTable.Rows.Add(new object[] { "Продължителност на обучението (в часове):", this.courseVM.MandatoryHours.HasValue && this.courseVM.SelectableHours.HasValue ? this.courseVM.MandatoryHours + this.courseVM.SelectableHours : "" });
            dataTable.Rows.Add(new object[] { "Цена (в лева за един курсист): ", this.courseVM.Cost });
            dataTable.Rows.Add(new object[] { "Други пояснения: ", this.courseVM.AdditionalNotes == null ? "" : this.courseVM.AdditionalNotes });
        ...
        private async Task SubmitBtn(bool showToast)
        {
            ...
            try
            {
                ...
                if (!this.validationMessages.Any())
                {
                    ...
                    {
                        await this.ShowErrorAsync("Не можете да създадете курс на обучение, тъй като в момента сте с временно отнета лицензия!");
                        this.SpinnerHide();
                        return;
                    }
                    ...
                    {
                        await this.ShowErrorAsync($"Не можете да запишете текущ курс с 'Дата на завършване на курса' {this.courseVM.EndDate!.Value.Year} г., защото има данни за подаден годишен отчет за {this.courseVM.EndDate!.Value.Year} г.!");
                        this.SpinnerHide();
                        return;
                    }
        ...
        private async Task CompleteTrainingCourseBtn()
        {
            ...
            try
            {
                ...
                bool isConfirmed = await this.ShowConfirmDialogAsync("Сигурни ли сте, че искате да приключите текущия курс?");
                if (isConfirmed)
                {
