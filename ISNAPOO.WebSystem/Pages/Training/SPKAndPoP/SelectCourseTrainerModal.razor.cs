...
{
    public partial class SelectCourseTrainerModal : BlazorBaseComponent
    {
        ...
        private List<TrainerCourseVM> addedTrainersSource = new List<TrainerCourseVM>();
        ...
        public async Task OpenModal(CourseVM courseVM, List<TrainerCourseVM> trainersSource)
        {
        ...
        private async Task TrainingTypeSelectedHandler(ChangeEventArgs<int, KeyValueVM> args)
        {
            ...
            if (args.Value != 0)
            {
                this.trainersSource = (await this.TrainingService.GetAllActiveCandidateProviderTrainersByIdTrainingTypeByIdSpecialityAndByIdCandidateProviderAsync(this.courseVM.IdCandidateProvider.Value, this.courseVM.Program.IdSpeciality, this.idTrainingType, this.kvTheoryAndPracticeId)).ToList();
