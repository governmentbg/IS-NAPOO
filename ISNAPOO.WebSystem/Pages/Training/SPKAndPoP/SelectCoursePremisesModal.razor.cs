...
{
    public partial class SelectCoursePremisesModal : BlazorBaseComponent
    {
        ...
        private List<PremisesCourseVM> addedPremisesSource = new List<PremisesCourseVM>();
        ...
        public async Task OpenModal(CourseVM courseVM, List<PremisesCourseVM> addedPremisesSource)
        {
        ...
        private async Task TrainingTypeSelectedHandler(ChangeEventArgs<int, KeyValueVM> args)
        {
            ...
            if (args.Value != 0)
            {
                this.premisesSource = (await this.TrainingService.GetAllActiveCandidateProviderPremisesByIdTrainingTypeByIdSpecialityAndByIdCandidateProviderAsync(this.courseVM.IdCandidateProvider.Value, this.courseVM.Program.IdSpeciality, this.idTrainingType, this.kvTheoryAndPracticeId)).ToList();
