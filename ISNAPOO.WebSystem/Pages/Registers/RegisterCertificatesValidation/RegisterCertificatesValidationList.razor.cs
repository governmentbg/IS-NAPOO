...
using ISNAPOO.WebSystem.Pages.Registers.RegisterLegalCapacity;
...
{
    public partial class RegisterCertificatesValidationList : BlazorBaseComponent
    {
        SfGrid<ClientCourseDocumentVM> sfGrid;

        RegisterCertificatesValidationModal modal;
        ...
        {
            //3-37В е docTypeOfficialNumber от таблица Request_TypeOfRequestedDocument по него филтрираме списъка
            var typeOfRequestedDocument = await this.providerDocumentRequestService.GetTypeOfRequestedDocumentAsyncByDocTypeOfficialNumber("3-37В");

            documents = await trainingService.GetAllDocumentsByIdTypeOfRequestedDocument(typeOfRequestedDocument.IdTypeOfRequestedDocument);
        }
