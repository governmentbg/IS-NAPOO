...
{
    public partial class RegisterLegalCapacityModal : BlazorBaseComponent
    {
        SfDialog sfDialog;
        ...
        [Inject]
        private IProviderDocumentRequestService ProviderDocumentRequestService { get; set; }
        [Inject]
        ...
        private TypeOfRequestedDocumentVM typeDocument = new TypeOfRequestedDocumentVM();

        public async Task openModal(ClientCourseDocumentVM clientCourseDocumentVM)
        {
            this.model = clientCourseDocumentVM;

            header1 = $"Информация за ЦПО: ЦПО {this.model.ClientCourse.Client.CandidateProvider.ProviderName} към {this.model.ClientCourse.Client.CandidateProvider.ProviderOwner}";
            ...
            {
                typeDocument = await ProviderDocumentRequestService.GetTypeOfRequestedDocumentByIdAsync(
                    new TypeOfRequestedDocumentVM()
                    {
                        IdTypeOfRequestedDocument = model.IdTypeOfRequestedDocument.Value
                    });
            }
